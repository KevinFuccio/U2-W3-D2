...
let arrPersonaggi = [
  ...
  },
  {
    ...
    role: "damage",
    image:
      "https://webcdn.hirezstudios.com/paladins/champion-icons/betty-la-bomba.jpg",
  },
  {
    name: "Bomb King",
    role: "damage",
    image:
      "https://webcdn.hirezstudios.com/paladins/champion-icons/bomb-king.jpg",
  },
  {
    name: "Buck",
    role: "flank",
  ...
  },
  {
    ...
    role: "damage",
    image:
      "https://webcdn.hirezstudios.com/paladins/champion-icons/sha-lin.jpg",
  },
  {
    name: "Skye",
    role: "flank",
...
class Champion {
  ...
  static counterChamp(array, id) {
    if ((id === nextBtn.id && this.arrEnd < array.length )|| id === "init") {
      if (array.length > this.arrStart) {
        divDisplay.innerHTML = "";
      }
      this.arrStart += 10;
      this.arrEnd += 10;
      array.slice(this.arrStart, this.arrEnd).forEach((el) => {
        let p = document.createElement("p");
        p.innerHTML = el.name;
        divDisplay.appendChild(p);
      });
    }else if(id === backBtn.id && this.arrStart >= 10){
        
        this.arrEnd = this.arrStart 
        this.arrStart -= 10
       
      if(array.length > this.arrEnd){
        divDisplay.innerHTML = "";
        }
        array.slice(this.arrStart,this.arrEnd).forEach((el) => {
          let p = document.createElement("p");
...
Champion.counterChamp(arrPersonaggi, "init")

nextBtn.addEventListener("click", (e) => {
  Champion.counterChamp(arrPersonaggi, e.target.id);
});
